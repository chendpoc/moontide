...
import type { SpillOptions } from "@moontide/session";
...
let workdir = resolvePath(env(APP_ENV.WORKDIR)?.trim() || process.cwd());
...
export function spillOptions(): SpillOptions {
  return {
    thresholdBytes: artifactSpillThresholdBytes(),
    previewMaxSummaryChars: toolPreviewChars(),
  };
}
