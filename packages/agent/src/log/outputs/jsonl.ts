...
export class JsonlWriter implements EventOutput {
	...
	constructor(options: JsonlWriterOptions = {}) {
		this.fixedWorkdir = options.workdir
		...
		this.gzip = options.gzip ?? ((input) => gzipBuffer(input));
	...
	handle(event: AgentEvent): void {
		...
		if (
			currentBytes > 0 &&
			currentBytes + serialized.bytes > this.segmentLimitBytes
		) {
	...
	finalizeRun(runId: string): void {
		const runsDir =
			this.runRoots.get(runId) ?? this.ensureStorage(this.resolveWorkdir());
	...
	private recoverSealedFiles(runsDir: string): void {
		for (const fileName of listDir(runsDir)) {
			...
			const archivePath =
				sealedPath.slice(0, -SEALED_EVENTS_SUFFIX.length) +
				ARCHIVE_EVENTS_SUFFIX;
			const tempPath =
				sealedPath.slice(0, -SEALED_EVENTS_SUFFIX.length) + TEMP_ARCHIVE_SUFFIX;
