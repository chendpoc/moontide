...
function spawnGit(workdir: string, args: string[]): Promise<{ stdout: string; stderr: string; code: number | null }> {
  return spawnCollect("git", args, { cwd: workdir });
}
...
export async function runGitStatus(workdir: string): Promise<string> {
  try {
    const sb = await spawnGit(workdir, ["status", "-sb"]);
    if (isNotGitRepo(sb.stderr)) {
      return JSON.stringify({ status: "error", error: "not a git repository" } satisfies GitStatusResult);
    }

    const porcelainOut = await spawnGit(workdir, ["status", "--porcelain"]);
...
export async function runGitDiff(workdir: string, input: GitDiffInput = {}): Promise<string> {
  ...
  if (input.path !== undefined && String(input.path).trim()) {
    try {
      args.push("--", safePath(String(input.path), workdir));
    } catch (error) {
      return JSON.stringify({
        status: "error",
        error: toMessage(error),
  ...
  try {
    const { stdout, stderr, code } = await spawnGit(workdir, args);
...
export async function runGitLog(workdir: string, input: GitLogInput = {}): Promise<string> {
  const n =
