...
import { joinPath, resolvePath } from "@moontide/shared/utils/path.js";
...
export const pythonRuntime: CodeRuntime = {
  ...
  async detect() {
    ...
    const configured = getToolsProductConfig().pythonPath();
    const venv = getToolsProductConfig().venvPath();
    const candidates = configured
      ? [configured]
      : [joinPath(venv ?? "", "bin", "python"), "python3", "python"].filter(Boolean);
