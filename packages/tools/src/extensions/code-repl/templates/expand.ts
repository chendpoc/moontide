...
import { safePath } from "../../../builtins/workspace/fs.js";
...
function resolveVars(
  ...
  for (const varDef of def.vars) {
    ...
    if (coerced === undefined) {
      ...
      if (varDef.default !== undefined) {
        ...
        if (varDef.path && typeof defaultVal === "string") {
          try {
            resolved[varDef.name] = safePath(defaultVal, workdir);
          } catch (error) {
            return {
    ...
    if (varDef.path && typeof coerced === "string") {
      try {
        resolved[varDef.name] = safePath(coerced, workdir);
      } catch (error) {
        return {
          error: toMessage(error),
          template: def.id,
...
export function expandTemplate(
  ...
  const resolvedResult = resolveVars(workdir, def, rawVars);
