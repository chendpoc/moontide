...
import type { ArtifactStore, SpillOptions } from "@moontide/session/stores";
...
function findInlineSpillCandidate(
  messages: Message[],
...
export async function enforceL3ReferenceBudget(input: EnforceL3Input): Promise<EnforceL3Result> {
  const { workdir, spillOptions } = input;
  ...
  for (let pass = 0; pass < working.length * 4; pass += 1) {
    const inlineTarget = findInlineSpillCandidate(working, spillOptions.thresholdBytes);
    if (!inlineTarget) {
      break;
    }
