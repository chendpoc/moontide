...
import { defaultBudgetConfig } from "../ports/budget-config.js";
...
function resolveFlexLimit(contextWindow: number, budget: BudgetConfig): number {
  if (!budget.flexEnabled) {
    return 0;
  }
  if (budget.l5Cap !== undefined) {
    return budget.l5Cap;
  }
  const pct = budget.flexPct ?? DEFAULT_FLEX_PCT;
...
function resolveTierCaps(contextWindow: number, budget: BudgetConfig) {
  const l1Default = budget.l1Cap ?? DEFAULT_L1_CAP;
  const l3Default = budget.l3Cap ?? DEFAULT_L3_CAP;
  if (contextWindow >= 128_000 || budget.l1Cap !== undefined || budget.l3Cap !== undefined) {
...
export function resolveBudgetPolicy(input: ResolveBudgetPolicyInput): BudgetPolicy {
  const budget = input.budget ?? defaultBudgetConfig;
  const contextWindow = input.modelProfile.contextWindow;
  const modelId = input.modelProfile.logicalModelId;

  const l4Limit = resolveL4Reserved(input.modelProfile, budget.l4Cap);
  const { l1Cap, l3Cap } = resolveTierCaps(contextWindow, budget);
  const l5Limit = resolveFlexLimit(contextWindow, budget);
