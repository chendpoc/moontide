...
import { DEEPSEEK_OPENAI_BASE_URL } from "@moontide/shared/constants/index.js";
...
function _toOpenAiMessages(request: LLMRequest): Array<{ role: "system" | "user" | "assistant"; content: string }> {
...
/** DeepSeek JSON mode via OpenAI Chat Completions (`response_format: json_object`). */
export async function deepseekOpenAiChat(
  request: LLMRequest,
  route: ResolvedRoute,
): Promise<LLMResponse> {
  ...
  const response = await fetch(`${DEEPSEEK_OPENAI_BASE_URL}/chat/completions`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    throw configError(`DeepSeek chat/completions failed (${response.status}): ${detail}`);
  }
