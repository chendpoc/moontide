...
function formatContext(context: Record<string, unknown>): string[] {
	...
	return parts.length > 0
		? [theme.label("  context ") + theme.context(parts.join("  "))]
		: [];
}
...
export function formatErrorTerminal(
	...
	const verbose = opts?.verbose ?? isDebugFileEnabled();
	...
	const locationParts = [
		...
		record.turn !== undefined
			? `turn ${String(record.turn).padStart(2, "0")}`
			: undefined,
	].filter(Boolean);

	const header = `${theme.marker(" ERROR ")} ${theme.code(record.code)}${
		locationParts.length > 0
			? theme.label(` · ${locationParts.join(" · ")}`)
			: ""
	}`;
...
export function formatPluginErrorEvent(event: {
	...
	const code = String(
		event.payload.errorCode ?? event.payload.code ?? "internal",
	);
	...
	const toolName =
		event.payload.toolName !== undefined
			...
			: undefined;
	const hook =
		event.payload.hook !== undefined ? String(event.payload.hook) : undefined;
