...
function _usageFromEvents(events: AgentEvent[]): {
	inputTokens: number;
	outputTokens: number;
} {
	let inputTokens = 0;
	...
	for (const event of events) {
		...
		const payload = event.payload as {
			report?: { usage?: { inputTokens?: number; outputTokens?: number } };
		};
...
function _setupHttpFixtures(
	...
	installEvalHttpFixtures(caseDef.httpFixturesPath, {
		record: options.recordHttpFixtures,
	});
	installEvalHttpReplay();
}
...
export async function runEvalCasePair(
