...
/** Trace event shape for derive layers (no log package dependency). */
...
interface BlockHandler {
  ...
  toTraceDraft?(block: ContentBlock, turn: number): TraceEventDraft | null;
}
...
export function traceDraftsFromBlocks(blocks: ContentBlock[], turn: number): TraceEventDraft[] {
  const drafts: TraceEventDraft[] = [];
