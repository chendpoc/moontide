...
| 3 | `tools` | 契约 | 无 | ☑ | ◐ | ◐ | RB1 完成；R4 集成待后续模块 |
| 4 | `event` | 契约 | 无 | ☑ | ☑ | ☑ | R1–R3 完成；R4 bus 待做 |
...
- 2026-08-16：结果载荷收敛：R1 删除无消费者且与 `ToolContent::Json` 重叠的 `structured`；`ToolOutput` / `ToolResult` 只保留单一 content。
...
- 2026-08-16：`ToolResult` 构造边界确认：字段跨 crate 只读；crate 内只保留 `new(call, status, content)` 与 `from_output(call, output)`，loop 决定管线状态，tools 统一组装身份与结果。
- 2026-08-16：单次调用内部 API 收口：registry 以 crate 内部 `validate_input(tool, call) -> Result<(), String>` 使用缓存 validator；`Tool::execute` 隐藏 executor 并直接返回规范化 `ToolResult`。
- 2026-08-16：executor 基础设施错误配对规则确认：loop 先提交一次 `OutcomeUnknown` 的 `ToolOutcomeRecorded`，再向 run 边界传播原始错误，禁止留下未配对 invocation。
