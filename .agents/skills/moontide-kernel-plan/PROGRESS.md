...
| 4 | `event` | 契约 | llm + tools 契约 | ☑ | ☑ | ☑ | commit-only；observability 整体后置 |
...
| 7 | `loop` | 编排 | 1–6 全部 | ◐ | ☐ | ☐ | 架构对齐中；先删除冗余 Run 抽象 |
...
- 当前推进：模块 7 `loop` 架构对齐；先完成冗余 Run 抽象删除批 Review，再基于 Session → Turn → Step → Tool round 模型复核公开契约。

## 变更记录

- 2026-08-17：loop 架构对齐删除冗余 Run 抽象：执行层级固定为 Session → Turn → Step → Tool round；`TurnEvent` 收敛为 Session commit 事实。同步删除 hook/observe/registry/context、Agent Event recorder 与纯观测事件；OTel、trace/span、bus、schema 与存储整体后置到真实接入。
...
- 2026-08-16：executor 参数收敛：删除 `ToolExecutionContext`；R1 只读借用 `&ToolCall` 并显式传入 `working_dir: &Path`，调用身份归 tools，trace/session/turn 身份留在高层模块。
