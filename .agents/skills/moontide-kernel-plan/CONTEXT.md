...
> Desktop process architecture（2026-08-20，用户确认）：目标拓扑为 `moontide-desktop` UI process → versioned `desktop-protocol` → `moontide-agent-host` runtime process。UI 只拥有 Iced window、RenderState、用户 intent 和 protocol client；Agent Host 独占 `Agent`、SessionStore、ApprovalBroker、Progress/diagnostic workers 和 runtime lifecycle。`agent-core::TurnEvent → agent::ProgressEvent → DesktopProtocolEvent → RenderState` 是三层事件语义；最终跨进程协议不暴露 Agent runtime ownership / implementation 类型，但 D1/D3 不强制复制全部 canonical value payload，in-process adapter 可以复用稳定的 `ToolCall`、`ToolResult`、`ContentBlock` 和 `ModelResponseSnapshot`。AgentLoop 仍是 Agent Host 内的 Tokio task，subagent 先是逻辑 runtime/actor；tool subprocess 只用于权限/隔离需要；daemon 是未来独立生命周期的 sibling/service，不是 Agent 子进程。D2 先冻结顶层 protocol 语义并提供 in-process adapter；只有 D4 出现真实 framed transport、独立版本或非 Rust consumer 时，才抽取必要 wire DTO。不实现 IPC、daemon、multi-agent 或 AgentLoop worker process，直到对应消费者明确出现。完整设计见 [`crates/docs/desktop-process-architecture.md`](../../../crates/docs/desktop-process-architecture.md)。
...
> Desktop UI framework selection（2026-08-20）：用户确认 Desktop v0.1 使用 Iced。Iced 只属于 Desktop UI 层，Host、`agent` 和 `agent-core` 不依赖 UI framework；UI 使用 `Message → update → view` 与 `Subscription/Task` 接入 `DesktopEventStream` 和 Host command。Slint 保留为历史调研候选但不进入实现合约；Tauri 只有真实 Web frontend / Web/Desktop 共用需求时重新评估；egui/eframe 仅作原型候选。

> Desktop UI resource direction（2026-08-20）：用户明确要求专业 Agent 产品必须保持资源与能力边界克制；Electron/内置 Chromium 形态作为 Desktop 产品架构排除，不只是 v0.1 后置。Tauri 虽使用 system WebView、不是 bundled Chromium，但仍引入 HTML/CSS/JavaScript、WebView 和 frontend/backend invoke，只有出现真实 Web frontend 或 Web/Desktop 共用需求才重新评估。Iced 已确认；固定内存/磁盘数字仍需 Iced-only 实测，不作为未经验证的事实写入设计；D3 进入资源、启动、渲染和关闭清理验收。
