...
> tools 设计修订（2026-08-16）：公开类型使用构造器与只读访问器；registry 构造即冻结、稳定排序并编译缓存 input validator，非法 schema 阻止整个 registry；RB1 使用关闭 default features 的 `jsonschema` 0.49，固定 Draft 2020-12 并禁用 HTTP/file resolver；registry 仅以 crate 内部 `validate_input(tool, call) -> Result<(), String>` 暴露预期参数错误，`Tool::execute` 隐藏 executor 并直接返回规范化 `ToolResult`；执行前仅保留 input validation 与 permission check，不引入阶段型 call、ToolAdmission 或 scheduler admission；R1 只保留 `input_schema`，`output_schema` 后置；tools 保留 canonical schema，LLM adapter 默认透传且只对已确认关键词异常做小型显式转换，不建设通用兼容层；调用 input 错误返回 `InvalidArguments`；retryable 不丢失；R1 status 不含 `Unavailable` / `TimedOut` / `InternalError`，基础设施错误走 `anyhow::Result`，loop 先提交 `OutcomeUnknown` 配对结果再传播原错误，timeout 待真实 producer 出现后再设计；结果载荷只保留 `ToolContent::Text | Json`，`ToolOutput` 直接以 `Succeeded` / `Failed { retryable }` / `OutcomeUnknown` enum 表达，不增加无消费者的 host-only `structured` 或独立 outcome enum；`ToolResult` 字段私有并只读访问，只提供 crate 内部 `new(call, status, content)` / `from_output(call, output)` 两个构造入口；删除无真实领域边界的 `ToolExecutionContext`，executor 只读借用 `&ToolCall` 并显式接收 `working_dir: &Path`，调用事实归 tools，run/session/turn 身份留在高层；删除无消费者且表达力不足的 `ToolExecutionPolicy`，scheduler 资源模型确认后再扩展边界；删除独立 permission 模块，permission 不进入 `ToolSpec`，组合根维护 `ToolPermissionMap<tool_name, Allow | Ask>` 并校验与 registry key 集完全一致，`loop` 私有查表且运行时缺失安全拒绝；完整拒绝顺序归 loop 集成测试；R4 由 session/event 持久化 typed ToolResultStatus，LLM 只接收模型可见 content。
