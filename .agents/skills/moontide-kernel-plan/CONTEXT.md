...
7. **llm 分层（2026-08-14，provider config 2026-08-26，四轴解耦 2026-09-03，R2–R5 2026-09-03）**：`agent-core::llm/protocol/` = MoonTide 协议；`AdapterFamily` 四协议（Chat / Responses / Anthropic Messages / Google Generative AI）均已实装 adapter + normalize；catalog 在 `agent::llm`（含 Custom provider `register_custom_providers`）；配置三层 L1 catalog → L2 `ResolvedProviderConfig` → L3 `LlmCallConfig`；连续性 Canonical + turn 内内存 `ContinuityHint.previous_response_id`（Optimized，不写 Session Item）。宿主：`MOONTIDE_PROTOCOL`、`MOONTIDE_RESPONSES_STORE`、settings `protocol`/`profile`/`custom_providers`（CLI v2 / Desktop v3）。Feature：[`crates/docs/features/LLM-FOUR-AXIS.md`](../../../crates/docs/features/LLM-FOUR-AXIS.md)。
