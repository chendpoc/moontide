...
| `session` | Session Item Log 的 create/load/commit/fork 与恢复不变量 | llm/tools 契约 + event commit seam | 已实现 |
...
- 项目级路径与 policy 见 [`design/logging-and-session-design.md`](design/logging-and-session-design.md)，R2 默认是 `SessionPersistence::Items + DiagnosticPersistence::Off`；
