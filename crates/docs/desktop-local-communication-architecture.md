...
The existing owners remain explicit: the Host `EventBuffer` assigns monotonic
`seq`; `DesktopRuntime` creates `connection_epoch`; and the Tauri event pump
maps internal events to WebView DTOs and coordinates listener-first startup,
snapshot baseline, gap detection, and resync. These are existing
responsibilities, not new actors or abstraction layers.
...
- Rust/TypeScript fixture or schema conformance continues to pass for every DTO
  that crosses the WebView boundary;
- existing Tauri capability, CSP, and security checks continue to pass, and the
  capability surface does not expand;
...
This document supersedes only the same-process request-routing requirements in
`tauri-protocol-boundary-refactor.md` and `desktop-process-architecture.md` that
require calls to traverse `DesktopProtocolClient`, `ClientTransport`, and
`DesktopProtocolServer`. Their Host ownership, event delivery, conformance,
security, and future process-boundary contracts remain current.
