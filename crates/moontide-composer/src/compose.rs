use moontide_protocol::{Message, SessionLog, ToolSchema};
use moontide_tools::{tool_definitions, ToolProjectionConfig};
...
impl ComposeOptions {
    pub fn from_env() -> Self {
        let auto_compact = std::env::var("MOONTIDE_COMPACT_AUTO")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(true);
        let compact_threshold = std::env::var("MOONTIDE_COMPACT_THRESHOLD")
            .ok()
...
fn estimate_messages_chars(messages: &[Message]) -> usize {
    moontide_tools::estimate_messages_chars(messages)
}
