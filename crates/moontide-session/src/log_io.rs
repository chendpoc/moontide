...
use chrono::Utc;
use moontide_protocol::{SessionLog, SessionLogBase, SessionLogBody};
...
pub fn parse_log(lines: &[String]) -> Result<Vec<SessionLog>> {
    ...
    for line in lines {
        ...
        if moontide_protocol::is_session_log(&value) {
    ...
    fn round_trip_user_and_assistant() -> Result<()> {
        ...
        let assistant = build_session_log(
            ...
            SessionLogBody::AssistantMessage {
                blocks: vec![moontide_protocol::ContentBlock::text("hi")],
            },
        );
