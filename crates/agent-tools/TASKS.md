...
- **做什么：** 物理分离 `grep` 的 Draft 2020-12 spec 与文件 executor；支持 pattern、相对 target 和 max_results，在 canonical working directory 内用 `ignore` + `regex` 完成稳定、有界、只读搜索。blocking IO 经 `spawn_blocking`，结果使用 `ToolOutput`，不实现 permission 或 RunEvent。
