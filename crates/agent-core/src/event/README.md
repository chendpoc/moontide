...
R2/R3：`derive_agent_event`、`DeriveObserveHandler`、`AgentEventWriter`、`FileAgentEventWriter`（`{runs_dir}/{run_id}.active.jsonl`）。writer 创建时扫描已有 active 文件，校验 `runId` 并恢复 `seq` 与最后 `turn`，追加后保持 `seq` 单调；作为路由键的 `run_id` 超过 128 bytes 时拒绝创建。
