...
> 首个 Review 批覆盖 tools 的单次调用基础能力；组合根声明的 `ToolPermissionMap` 与查表顺序由后续 loop 集成批验证，不引入独立 permission 模块或 scheduler 的多调用策略，也不提前修改 session/event/llm 实现。
> executor `Err` 的配对规则属于 R4 loop/event/session 集成：RB1 只验证 `Tool::execute` 原样传播错误；R4 必须先提交 `OutcomeUnknown`，再把同一错误返回 run 边界。
...
- **做什么：** 实现只含模型能力声明的 `ToolSpec`、`ToolCall`、enum `ToolOutput`、私有字段且只读访问的 `ToolResult` 及其状态/内容类型；`ToolResult` 只提供 crate 内部 `new(call, status, content)` 与 `from_output(call, output)` 两个构造入口。实现唯一的 `ToolExecutor` trait，调用以 `&ToolCall` 只读借用传入，工作目录通过显式 `working_dir: &Path` 参数传入。类型层不执行 IO，不依赖高层模块，不包含 permission map，也不预设 scheduler 执行策略。
- **依赖：** 无
...
- **做什么：** 实现调用身份、缓存的 Draft 2020-12 input 校验、executor 调用以及 `ToolOutput → ToolResult` 规范化。registry 仅增加 crate 内部 `validate_input(tool, call) -> Result<(), String>`；`Tool::execute` 隐藏 executor 并直接返回规范化 `ToolResult`。RB1 只验证 `resolve` 未命中、validation error 和结果构造能力；`UnknownTool` / `InvalidArguments` 的完整管线映射与“executor 未调用”顺序由未来 loop 集成测试负责。只处理一个 `ToolCall`；不实现 output schema、provider schema 转换、permission、admission、并发、取消树、重试或 offload 验收。
...
| `ToolOutput → ToolResult` 保留状态和 retryable | result / execute 行为测试 |
