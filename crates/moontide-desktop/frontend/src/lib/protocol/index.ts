...
export const DesktopCommandSchema = z.discriminatedUnion("kind", [
  ...
  z.strictObject({ kind: z.literal("list_sessions") }),
  z.strictObject({ kind: z.literal("new_chat") }),
...
const SessionCatalogRowSchema = z.strictObject({
  session_id: z.string(),
  first_user_message_excerpt: z.string().nullable(),
  last_activity_at: z.string().nullable(),
  loaded: z.boolean(),
});

const SessionCatalogRowsSchema = z.array(SessionCatalogRowSchema).superRefine((rows, context) => {
  if (rows.filter((row) => row.loaded).length > 1) {
    context.addIssue({
      code: "custom",
      message: "Session catalog must not contain more than one loaded Session",
    });
  }
});
...
export const DesktopResponseSchema = z.discriminatedUnion("kind", [
  ...
  z.strictObject({
    kind: z.literal("session_catalog_listed"),
    connection_epoch: uint,
    rows: SessionCatalogRowsSchema,
  }),
  z.strictObject({
    kind: z.literal("generation_ready"),
    connection_epoch: uint,
  }),
  z.strictObject({
...
export function parseDesktopResponse(input: unknown): DesktopResponse {
  return DesktopResponseSchema.parse(input);
}
