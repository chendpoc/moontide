...
use crate::event::{DesktopEvent, EventBuffer};
...
impl ApprovalBroker {
    ...
    pub(crate) fn approve(
        &self,
        request_id: &str,
    ) -> std::result::Result<(), crate::DesktopCommandError> {
    ...
    fn resolve(
        ...
        let Some(pending) = pending else {
            return if lock_resolved(&self.resolved).contains(request_id) {
                Err(crate::DesktopCommandError::ApprovalAlreadyResolved)
            } else {
                Err(crate::DesktopCommandError::ApprovalNotFound)
            };
        };
        ...
        *self
        ...
        pending
            .sender
            .send(decision)
            .map_err(|_| crate::DesktopCommandError::ApprovalAlreadyResolved)
    }
}
...
mod tests {
    use super::*;
    use crate::event::DesktopEventStream;
