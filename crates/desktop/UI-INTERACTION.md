...
```text
creating → handshaking → ready → consumed/running → stopping → stopped
```

- Ready generation 已完成 handshake，尚未消费 one-shot `StartSession`。
- 首次有效 StartSession 无论成功或失败都会消费当前 generation。
...
validate draft
  ...
  → StartSession(New)
  → wait for SessionReady snapshot with loaded identity
  → SubmitTurn(exact draft) once
  → clear submitted draft after TurnAccepted
```
...
4. StartSession rejection 或 transport failure：仍是 Blank，恢复 exact draft；该 generation
   已被消费/关闭，进入 runtime unavailable，直到 Retry 创建并 handshake fresh generation。
5. StartSession accepted、SubmitTurn rejected：保持 Loaded 与 exact draft。
6. TurnAccepted 后只清除已提交文本；用户在 in-flight 期间新输入的内容不被清除。
7. resync 期间暂停 sequence；只有 authoritative snapshot 能决定继续或失败。
8. Session 创建/加载的接受点是 `SessionReady`，不是后续 `TurnAccepted`。
9. component 不等待 timeout，也不自行串接两条 bridge command。
...
keep Loaded visible and preserve draft
  ...
  → create + handshake fresh generation
```

- shutdown rejection/failure：保留 Loaded 与 draft。
- shutdown 成功后不回滚到旧 Loaded。
- fresh creation/handshake failure：保持 Blank，显示 Retry，Composer disabled。
...
2. first Send start failure 保留 exact draft且不建立 loaded identity。
3. first Send submit rejection 保留 Loaded 与 exact draft。
4. double activation 只产生一次 StartSession/SubmitTurn sequence。
