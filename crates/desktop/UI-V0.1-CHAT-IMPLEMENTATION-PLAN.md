...
> **状态：** Confirmed；Batch 0 Contract reset 等待用户 review
...
> **本批范围：** 只交付开发实现方案；不修改 Rust、Svelte、protocol、lockfile 或现有 UI contract
...
`crates/desktop/references/chat-ui/`：`unsloth-blank.png`、`unsloth-loaded.png`、
...
- **Session catalog contract：** Session sidebar 需要可恢复的 list/load/new contract；不能用 frontend fixture
  冒充生产 Session list。
- **first-send lifecycle：** 必须明确 Blank 页第一次 Send 是先 start Session 再 submit Turn，还是由一个
  controller transaction 编排两个现有命令；失败时不能创建 UI-only loaded Session identity 或丢失 draft。
- **one-shot server lifecycle：** 当前 protocol server 的首次有效 `StartSession` 会消费 `AgentConfig`，同一
  server 不能创建第二个 Agent。`New Chat` 与 Session switch 必须由 composition root / Tauri bootstrap
  coordinator 显式关闭旧 generation、创建并 handshake 新 server；不得在 Svelte 或 adapter 内隐藏重建。
...
composition root 持有一个完成 handshake、尚未消费 `StartSession` 的 `Ready` generation；server 创建或
handshake 失败时，页面仍是带 recoverable notice 的 Blank，但 Composer 禁用。protocol server generation、
...
check close gate while keeping Loaded visible
  ...
  → create a fresh protocol server generation
  → Handshake to Ready
```

- shutdown 确认前保持 Loaded page 与 draft，不提前显示 Blank；
- shutdown 失败时保留最后一个 Loaded projection，并显示明确的 disconnected/retry notice；
- shutdown 已确认成功后必须清除 loaded Session identity；server creation 或 handshake 失败时保持 Blank，
...
controller-owned sequence：

```text
validate draft
  → request/start Session
  → wait for accepted loaded Session identity
  → submit Turn exactly once
  → on success clear submitted draft
```

- start failure：保持 Blank 与 exact draft；当前 generation 已被消费并关闭，Retry 必须由 composition
  root 创建并 handshake fresh generation 后才重新允许 first Send；
...
Session 的创建/加载接受点是 `StartSession` 返回的 accepted loaded identity，而不是后续
`TurnAccepted`。因此 StartSession 成功但 SubmitTurn 被拒绝时，保留一个 Loaded、可能尚无 message 的
Session；不得回滚为 Blank 或删除已创建的 Session。
...
- 当前已有 loaded Session：先执行与 New Chat 相同的 close/recreate/handshake transaction；旧内容保持可见到
...
- 将四张 UI reference 固化到 `crates/desktop/references/chat-ui/` 并记录 provenance 与状态映射；
- 不改代码。
...
- 由 composition root / Tauri bootstrap coordinator 实现并测试 server generation 的
  shutdown → recreate → handshake 顺序；
...
- 使用 `crates/desktop/references/chat-ui/` 的四张 reference 与 implementation capture 做同状态比较；
