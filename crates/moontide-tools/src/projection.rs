...
pub fn estimate_messages_chars(messages: &[moontide_protocol::Message]) -> usize {
    use moontide_protocol::{ContentBlock, MessageContent};
    messages
        .iter()
            ...
            MessageContent::Blocks(blocks) => blocks
                ...
                .map(|b| match b {
                    ...
                    ContentBlock::ToolResult { content, .. } => match content {
                        moontide_protocol::ToolResultContent::Text(t) => t.len(),
                        moontide_protocol::ToolResultContent::Blocks(blocks) => blocks
                            .iter()
                            .map(|b| match b {
                                ContentBlock::Text { text } => text.len(),
                                _ => 0,
