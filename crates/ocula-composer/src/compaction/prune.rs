use ocula_protocol::{ContentBlock, Message, MessageContent, Role};
        ...
        .map(|block| match block {
            ContentBlock::ToolResult {
                ...
                let body = match content {
                    ocula_protocol::ToolResultContent::Text(t) => t,
                    ...
                    || body.starts_with(COMPACT_PLACEHOLDER_PREFIX)
                {
                    ContentBlock::ToolResult {
                        tool_use_id,
                        content: ocula_protocol::ToolResultContent::Text(body),
                    }
                } else {
                    truncated += 1;
                    ContentBlock::ToolResult {
                        tool_use_id,
                        content: ocula_protocol::ToolResultContent::Text(shrink_tool_result_content(
                            &body,
                        )),
                    }
                }
...
pub fn estimate_percent_used(messages: &[Message], system: &str, context_limit: usize) -> u32 {
    let chars = ocula_tools::estimate_messages_chars(messages) + system.len();
    if context_limit == 0 {
        return 0;
    }
...
mod tests {
    use super::*;
    use ocula_protocol::ToolResultContent;
