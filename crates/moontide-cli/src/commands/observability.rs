...
use moontide_observability::{ObservabilityState, ToggleParse, parse_toggle};
    ...
    match parse_toggle(arg) {
        ...
        ToggleParse::Status => {
            ...
            lines.push(if obs.is_thinking_enabled() {
                ...
                "thinking off — enable with /thinking on or MOONTIDE_THINKING=1".into()
            });
            lines.join("\n")
        }
        ToggleParse::On => {
    ...
    match parse_toggle(arg) {
        ...
        ToggleParse::Status => {
            ...
            lines.push(if obs.is_verbose_enabled() {
                ...
                "verbose off — enable with /verbose on or MOONTIDE_VERBOSE=1".into()
            });
            lines.join("\n")
        }
        ToggleParse::On => {
