...
> **分工：** Desktop IPC / sidecar 候选见 [`runtime-multilang.md`](../notes/runtime/runtime-multilang.md)；竞品 context 机制见 [`context-analysis.md`](../notes/context/context-analysis.md)；历史 Hook 候选见 [`agent-run-hooks.md`](../archive/notes/runtime/agent-run-hooks.md) §11+；插件加载与 MCP 集成见 [`plugin-host.md`](../notes/runtime/plugin-host.md)。观测与 Hook 当前均后置。
...
flowchart TB
  ...
  MoonTideUI -.->|"观测接缝后置"| MoonTideCLI
  Sidecar -->|"MoonTide Plugin SDK"| Sidecar
  MCP -->|"stdio or HTTP"| ExtServers["External MCP servers"]
```

**内核职责（Rust）：** agent loop、Session append/read、Context materialize、ModelRequest compile、LLM HTTP adapter、内置 tools、MCP client、权限 broker。观测协议与存储待真实 UI/诊断接入时设计。
...
**Hook：** 当前后置；[`agent-run-hooks.md`](../archive/notes/runtime/agent-run-hooks.md) 仅作历史候选参考，不是 Rust 契约。
...
| [`agent-run-hooks.md`](../archive/notes/runtime/agent-run-hooks.md) | 历史 Hook 候选（当前后置） |
| [`plugin-host.md`](../notes/runtime/plugin-host.md) | Plugin host、MCP attach、tool registry |
| [`event/DESIGN.md`](../../crates/agent-core/src/event/DESIGN.md) | 当前 Rust Turn 级观测边界 |
