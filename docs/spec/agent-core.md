...
**MoonTide 落盘：** 开发计划 [`agent-core-roadmap.md`](../notes/runtime/agent-core-roadmap.md) · 术语 [`AGENTS.md`](../../AGENTS.md) §7.2（**RunEvent bus** · **resolveRunConfig** · **resolveTurnContext**；不用 sink / fold）。
...
2. **不做运行时钩子注册（core 层）**。`@moontide/agent-core` 没有注册表、没有热插拔；`RunConfig` 在 run 开始前装配并冻结。**MoonTide Harness（`@moontide/agent`）** 另有 `RunObserverRegistry`：sidecar 在 run 前注册 observer phase（`beforeToolUse`、`llmCall` 等），run 内不可增删；sidecar **不能**定义新 RunEvent phase（见 [`AGENTS.md`](../../AGENTS.md) §7.2）。
