...
- **Model routing**（任务 → 选模型 + thinking）是 MoonTide 的差异化能力。
...
4. **Loop 不绑厂商** — agent 内核只依赖 MoonTide 协议类型 + `LLMProvider` 接口；SDK 类型关在 adapter 内。
...
- `MODEL_ID` + 可选 `MOONTIDE_MODEL_FALLBACKS`
...
|-----------|------|--------|------|--------|
| `custom` | 用户自建的 OpenAI 形或 Anthropic 形中转站 | 用户选 `anthropic-messages` 或 `openai-chat-completions` | `CUSTOM_API_KEY` + `MOONTIDE_CUSTOM_BASE_URL` + `MOONTIDE_CUSTOM_ADAPTER` | 是 |
...
flowchart TB
  subgraph user [用户 / CLI]
    ...
    Auto["自动：MOONTIDE_MODEL_ROUTING=auto"]
  end

  subgraph core [MoonTide agent 内核]
    Loop["agent/loop"]
    Context["context/*"]
    Tools["tools"]
  end
...
| 产品 | Harness | API 适配 | MoonTide 定位 |
|------|---------|----------|--------------|
| **Pi Agent** | 自研 loop + `Context` 中间态 | 4 协议自研 HTTP | Harness 同档；adapter 层用方案 A 更省事 |
| **OpenCode / Cherry** | 自有 session / event | Vercel AI SDK + `ProviderTransform` | MoonTide **不用** `ai` 包作 loop 抽象 |
| **OpenRouter** | 无 agent harness | 网关 OpenAI Chat 形 | MoonTide 的一个 preset，非产品核心 |
...
仅需新增 `adapters/` 实现或 compat 配置行，**不改** `agent/loop` 与 MoonTide 协议。
...
按 `presetId` 查表；`custom` 可允许用户通过 env 或 `.moontide/compat.json` 覆盖（实现细节待定）。
...
| `openai` | OpenAI `tool_calls` ↔ MoonTide `tool_use` / `tool_result` |
| `gemini` | Gemini function call 块 ↔ MoonTide 块 |
...
1. 若用户显式 `MOONTIDE_PROVIDER=xxx` → 使用该 preset（若 model 注册表有对应 route）。
2. 若自动路由 → 按 `prefer` 顺序，选 **第一个有 API key 的 preset**。
3. 若仅 `OPENROUTER_API_KEY` → 可走 OpenRouter route。
4. `MOONTIDE_OPENROUTER_FALLBACK=1`：无官方 key 时自动落到 OpenRouter。
...
来源优先级：**model 注册表** → env 覆盖（`MOONTIDE_CONTEXT_LIMIT`）→ default。Context Composer 的预算与 token 策略来自 **ModelProfile**；见 [`context-composer.md`](context-composer.md)。
...
**放置位置：** MoonTide agent loop，**在 `runLLM` 之前**（`src/llm/router/` 或 `src/agent/routing/`）。
...
|------|------|--------|
| 0 | 零成本启发式 | — |
| 1 | 意图 / 路由分类 | `local-direct` → `moontide/router-v1` |
| 2 | 轻量多模态 / 闲聊 | `local-direct` → `moontide/general-v1`（按需加载） |
...
| 项 | 含义 | 今天 |
|----|------|------|
| `MOONTIDE_THINKING` / `/thinking` | **终端 trace** 是否展示 thinking 块 | 已有 |
...
```bash
MOONTIDE_PROVIDER=deepseek    # deepseek|kimi|openai|anthropic|gemini|openrouter|custom
...
```bash
MOONTIDE_PROVIDER=openrouter
OPENROUTER_API_KEY=sk-or-...
MODEL_ID=anthropic/claude-sonnet-4
# MOONTIDE_MODEL_FALLBACKS=openai/gpt-4o,deepseek/deepseek-chat
```

### 11.3 用户自定义中转（custom）

```bash
MOONTIDE_PROVIDER=custom
MOONTIDE_CUSTOM_ADAPTER=openai-chat-completions   # 或 anthropic-messages
MOONTIDE_CUSTOM_BASE_URL=https://relay.example/v1
...
```bash
MOONTIDE_MODEL_ROUTING=auto
# 可选：无官方 key 时允许 OpenRouter
MOONTIDE_OPENROUTER_FALLBACK=1
OPENROUTER_API_KEY=sk-or-...
```
...
```bash
MOONTIDE_CONTEXT_LIMIT=1000000
MOONTIDE_CONTEXT_EXACT=0
```

### 11.6 本地推理（演进，见 edge-local-models）

```bash
MOONTIDE_LOCAL_INFER=on              # ability 总开关；off 时 Router 不选 local tier
MOONTIDE_LOCAL_ROUTER=moontide/router-v1
MOONTIDE_LOCAL_GENERAL=moontide/general-v1
# sidecar 默认 ~/.moontide/infer.sock；本地 model catalog 权重 ~/.moontide/models/
```
...
| 阶段 | 内容 | 验收 |
|------|------|------|
| **A** | MoonTide 协议类型；SDK import 收到 adapter 白名单 | typecheck + test 全绿，行为不变 |
