# Ocula LLM Provider 与 Model 机制

> Ocula 如何把「选模型、接 API、发请求」从 agent loop 里拆出来。  
...
4. **Loop 不绑厂商** — agent 内核只依赖 Ocula 协议类型 + `LLMProvider` 接口；SDK 类型关在 adapter 内。
...
- `MODEL_ID` + 可选 `OCULA_MODEL_FALLBACKS`
...
|-----------|------|--------|------|--------|
| `custom` | 用户自建的 OpenAI 形或 Anthropic 形中转站 | 用户选 `anthropic-messages` 或 `openai-chat-completions` | `CUSTOM_API_KEY` + `OCULA_CUSTOM_BASE_URL` + `OCULA_CUSTOM_ADAPTER` | 是 |
...
flowchart TB
  subgraph user [用户 / CLI]
    ...
    Auto["自动：OCULA_MODEL_ROUTING=auto"]
  end
...
Ocula 将 **Harness（loop + 中间态）** 与 **API 适配层（adapter 层）** 严格分离：后者负责把 `LLMRequest` 译为厂商 HTTP API 并发包（adapter + normalize + 官方 SDK）。API 适配层采用 **方案 A**，Harness 控制力可与 Pi Agent 同档——控制力来自自有 ModelInput / loop，不来自是否自研 HTTP。
...
- **调试直观**：主路径读厂商官方 SDK 文档与类型；比整包 `ai` 的 `streamText` agent 范式更易与 Ocula 自研 loop 对齐。
...
| 产品 | Harness | API 适配 | Ocula 定位 |
|------|---------|----------|--------------|
| **Pi Agent** | 自研 loop + `Context` 中间态 | 4 协议自研 HTTP | Harness 同档；adapter 层用方案 A 更省事 |
| **OpenCode / Cherry** | 自有 session / event | Vercel AI SDK + `ProviderTransform` | Ocula **不用** `ai` 包作 loop 抽象 |
| **OpenRouter** | 无 agent harness | 网关 OpenAI Chat 形 | Ocula 的一个 preset，非产品核心 |
...
仅需新增 `adapters/` 实现或 compat 配置行，**不改** `agent/loop` 与 Ocula 协议。
...
按 `presetId` 查表；`custom` 可允许用户通过 env 或 `.ocula/compat.json` 覆盖（实现细节待定）。
...
| `custom` | 用户选 anthropic 或 openai 形 | 同上 | `OCULA_CUSTOM_BASE_URL` |
...
| `openai` | OpenAI `tool_calls` ↔ Ocula `tool_use` / `tool_result` |
| `gemini` | Gemini function call 块 ↔ Ocula 块 |
| `handoff` | 换 preset/model 时清洗不兼容 block |
| `stream` | 统一 stream 事件供 trace / UI（分期） |

Adapter **只做** Ocula 协议 ↔ 厂商 SDK 请求/响应形状；语义转换优先走 normalize，避免每个 preset 复制逻辑。
...
[`runLLM`](../../src/agent/pipeline/runLLM.ts) 目标形态：接受/返回 Ocula `LLMRequest` / `LLMResponse`；loop 判断 `stopReason === "tool_use"`，不再依赖 SDK 字段名 `stop_reason`。
...
**放置位置：** Ocula agent loop，**在 `runLLM` 之前**（`src/llm/router/` 或 `src/agent/routing/`）。
...
| 项 | 含义 | 今天 |
|----|------|------|
| `OCULA_THINKING` / `/thinking` | **终端 trace** 是否展示 thinking 块 | 已有 |
...
```bash
OCULA_PROVIDER=openrouter
OPENROUTER_API_KEY=sk-or-...
MODEL_ID=anthropic/claude-sonnet-4
# OCULA_MODEL_FALLBACKS=openai/gpt-4o,deepseek/deepseek-chat
```

### 11.3 用户自定义中转（custom）

```bash
OCULA_PROVIDER=custom
OCULA_CUSTOM_ADAPTER=openai-chat-completions   # 或 anthropic-messages
OCULA_CUSTOM_BASE_URL=https://relay.example/v1
...
```bash
OCULA_MODEL_ROUTING=auto
# 可选：无官方 key 时允许 OpenRouter
OCULA_OPENROUTER_FALLBACK=1
OPENROUTER_API_KEY=sk-or-...
```
