...
> 与 **thinking / verbose**（美化摘要、有截断）并列的第三档观测：**无截断全量** compose / llm_call / tool_use。实现位于 [`context-inspect/`](../../../packages/agent/src/context-inspect/) + hook 适配 [`debug-hook-module`](../../../packages/agent/src/plugins/builtin/context/debug-hook-module.ts)。
