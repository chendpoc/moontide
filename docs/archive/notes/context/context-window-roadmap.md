...
> **Hook：** [`agent-run-hooks.md`](../runtime/agent-run-hooks.md) · **Agent Event：** [`agent-events.md`](../../spec/agent-events.md) · **Utils：** [`utils-infrastructure.md`](../runtime/utils-infrastructure.md)
...
| **8.3** | [`edge-local-models.md`](../llm/edge-local-models.md) · [`llm-provider.md`](../../spec/llm-provider.md) §3.4 |
...
| **7c** | **Explore MVP（无完整 subagent）** | 轻量：单次 run 内并行 tool batch，或 sidecar 固定 prompt + bounded 摘要回传；完整 fork/fresh subagent 与 [`session-handoff.md`](../session/session-handoff.md) 对齐 | backlog |
...
与功能轨独立的 **架构修复 PR 序列**，详见 [`architecture-remediation.md`](../runtime/architecture-remediation.md)。
...
1. 本文（工作项 + 现状）
2. [`session-domain-model.md`](../session/session-domain-model.md) — 类型与数据流
3. [`agent-run-hooks.md`](../runtime/agent-run-hooks.md) — HookDispatcher · phase · sidecar
4. [`utils-infrastructure.md`](../runtime/utils-infrastructure.md) — fs / process / storage 分层
5. [`ecosystem-compat.md`](../runtime/ecosystem-compat.md) — MCP/Codex 兼容
...
| **2** | [`agent/hooks/`](../../../packages/agent/src/agent/hooks/) | phases · dispatcher · registry · defaults |
| **3** | [`run-event-derive.ts`](../../../packages/agent/src/log/run-event-derive.ts) · [`tool-use-log/`](../../../packages/agent/src/plugins/builtin/tool-use-log/) | RunEvent → AgentEvent（legacy log-sync 已删） |
| **4** | [`instruction-state/`](../../../packages/agent/src/instruction-state/) | load · resolve · epoch |
...
| 组件 | 状态 |
|------|------|
| `HookDispatcher` + `PHASE_DEFS` | done — [`src/agent/hooks/`](../../../packages/agent/src/agent/hooks/) |
...
Sidecar transport：**stdio pipe IPC**（非 HTTP）；终局 UDS 见 [`plugin-host.md`](../runtime/plugin-host.md)。
...
- `bus.ts` → **`event-hub.ts`**
- **Utils 基础设施抽离** — 见 [`utils-infrastructure.md`](../runtime/utils-infrastructure.md)
...
| Plugin host `kind: sidecar` | done — [`plugin-host.md`](../runtime/plugin-host.md) |
| Utils / storage 分层 | done — [`utils-infrastructure.md`](../runtime/utils-infrastructure.md) |
...
| 文档 | 关系 |
|------|------|
| [architecture-remediation.md](../runtime/architecture-remediation.md) | Phase A–C 架构修复 |
| [context-composer.md](../../spec/context-composer.md) | C0–C6 Spec |
| [agent-run-hooks.md](../runtime/agent-run-hooks.md) | Hook 生命周期 |
| [utils-infrastructure.md](../runtime/utils-infrastructure.md) | fs / process / event-hub |
| [session-log-migration.md](../session/session-log-migration.md) | C1b、runtime-status |
...
| [edge-local-models.md](../llm/edge-local-models.md) | §8.3 Local Fusion |
