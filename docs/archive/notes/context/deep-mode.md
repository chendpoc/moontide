...
- Gate：[`src/agent/deep-mode.ts`](../../../packages/agent/src/agent/deep-mode.ts) · REPL 调用 [`src/cli/repl/run.ts`](../../../packages/agent-cli/src/cli/repl/run.ts)
- Tool：`work_mem` → [`src/plugins/builtin/work-mem/`](../../../packages/agent/src/plugins/builtin/work-mem/)
...
Compose 路径（[`compose-for-turn.ts`](../../../packages/agent/src/agent/compose-for-turn.ts) → [`working-set-compose.ts`](../../../packages/agent/src/agent/working-set-compose.ts)）在 deep mode 下：
...
| 层 | 职责 |
|----|------|
| [`agent/deep-mode.ts`](../../../packages/agent/src/agent/deep-mode.ts) | REPL 状态 · `deep:` gate · activeWorkMemId |
| [`agent/ports/work-mem.ts`](../../../packages/agent/src/agent/ports/work-mem.ts) | 端口：`startDeepTaskRecord` · `resolveWorkingSetSnapshot` |
| [`plugins/builtin/work-mem/register.ts`](../../../packages/agent/src/plugins/builtin/work-mem/register.ts) | 注册 store + escalation 实现（bootstrap / register-defaults） |
...
| [`agent-eval-roadmap.md`](../evals/agent-eval-roadmap.md) | Feature 评测：**deep_protocol（B 桶）** protocol grader · Impact Card |
