...
Hook observe 返回值中的 `EventDraft` 由 `HookDispatcher` 统一 `emitDraft`；Session 派生见 `packages/agent/src/log/run-event-derive.ts`。
...
| 文档 | 关系 |
|------|------|
| [`context-window-roadmap.md`](../context/context-window-roadmap.md) | #6 cleanup 含 deprecated + utils 抽离 |
