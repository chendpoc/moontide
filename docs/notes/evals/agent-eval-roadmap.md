...
> **关联：** [`agent-eval-task-taxonomy.md`](agent-eval-task-taxonomy.md)（workload + Harness contract 分类依据） · [`harness-eval-refactor-plan.md`](harness-eval-refactor-plan.md)（contract-first 重构） · [`deep-mode.md`](../context/deep-mode.md) · [`deep-mode-redesign.md`](../context/deep-mode-redesign.md) · [`web-content-retrieval-discussion.md`](../context/web-content-retrieval-discussion.md) · [`agent-events.md`](../../spec/agent-events.md) · [`context-inspect-debug.md`](../context/context-inspect-debug.md)
...
**基础设施失败**（artifact 嵌套、HTTP 429）单独计 **infra_penalty**，与 feature 分解耦（见 [`web-content-retrieval-discussion.md`](../context/web-content-retrieval-discussion.md)）。
...
- [ ] eval runner：独立 CLI vs `pnpm test` 子命令？ → 见 [`eval-release-artifact.md`](eval-release-artifact.md) §4–§5（`eval-run` + `AgentArtifactCommand`）  
- [ ] baseline 存储：repo 内 JSON vs CI artifact 库？ → 见 [`eval-release-artifact.md`](eval-release-artifact.md) §7–§8  
- [ ] DSBench 公开后是否直接接入 L3？  
- [ ] Semantic config 与 eval manifest 如何一一对应？ → 见 [`eval-release-artifact.md`](eval-release-artifact.md) §9  
...
| 文档 | 关系 |
|------|------|
| [`eval-release-artifact.md`](eval-release-artifact.md) | L2 agent artifact · headless 协议 · intervention kinds · 迁移 R0–R5 |
| [`harness-eval-1.0.md`](../../spec/harness-eval-1.0.md) | 现行 Spec |
| [`harness-eval-refactor-plan.md`](harness-eval-refactor-plan.md) | contract-first · `EvalRunObservation` · **R0 必须先对齐** |
...
| 日期 | 说明 |
|------|------|
| 2026-08 | 链到 eval-release-artifact 第二稿（headless 协议 · intervention kinds） |
