...
| 维度 | 说明 |
|------|------|
| **已有能力** | `grep` 仅搜**工作区**本地文件（`rg`/`grep`）；[`prompt.ts`](../../../packages/agent/src/agent/prompt.ts) 已写 Prefer grep over bash for **code search** |
...
| 2026-08 | 关联 [`agent-eval-roadmap.md`](../evals/agent-eval-roadmap.md) D 桶 infra_penalty |
