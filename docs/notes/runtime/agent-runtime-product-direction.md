...
flowchart TB
  Model["基础模型：理解、推理、生成"]
  Core["MoonTide Agent Core：Turn、Step、Tool、Policy、Context、Event、Recovery"]
...
- `TurnEvent` commit protocol；观测 bus 后置；
- abort、队列、settlement、错误边界和恢复语义；
- 通过 Effect port 调用模型、工具和持久化能力；
- 为持久化提供稳定 Turn 事实；UI、CLI、RPC 和评测的观测接缝后置。
...
- 因为产品愿景而提前改变现有 `Session Item Log`、context、`TurnEvent` 等正式技术边界。
