...
> **设计 Spec：** [`docs/spec/agent-core.md`](../../spec/agent-core.md) · **术语：** [`AGENTS.md`](../../../AGENTS.md) §7.2  
...
| 包 | 职责 |
|----|------|
| `@moontide/run-protocol` | 共享契约：`protocol/`（RunEvent、RunConfig、Effect 端口） |
...
```mermaid
flowchart TB
  subgraph common ["@moontide/run-protocol"]
...
| [`agent-core.md`](../../spec/agent-core.md) | 标注 agent-common/core 实现对照 |
...
- MoonTide legacy：[`src/agent/agent-run.ts`](../../../packages/agent/src/agent/agent-run.ts) · [`src/log/event-hub.ts`](../../../packages/log/src/event-hub.ts)

---

## 12. 后续：根 `src/` monolith 消除（TODO §17）

**目标：** 不是删掉 MoonTide，而是 **Modular monorepo + package by bounded context** — 按域迁入 `packages/*`，入口缩到 `packages/agent-cli`，**删除根 `src/` monolith**。

| 阶段 | 内容 |
|------|------|
| **§16** | 先 `agent-common` + `agent-core`；根 `src/` 仍承载 harness |
| **§17** | 迁 `session` · `context-composer` · `llm` · `plugin-host` · `tools` · `cli`；架构边界测试扩到 package 级 |
| **终局** | `packages/agent-cli` 只 Preset 装配 + REPL；无根 `src/` |
