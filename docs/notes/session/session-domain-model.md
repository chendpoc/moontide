...
| 路径 | 职责 | 计划变更 |
|------|------|----------|
| [`src/session/session.ts`](../../../packages/session/src/session.ts) | 持有 `messages[]`，`append*`，经 port 落盘 | **`SessionItemCommitPort`**（[架构修复 §1](../runtime/architecture-remediation.md) · 已实现） |
| [`src/session/transform/`](../../../packages/session/src/transform/) | `messagesFromItems` · `itemsFromMessages` · `messagesFromContext` | — |
| [`packages/session/src/item-handlers.ts`](../../../packages/session/src/item-handlers.ts) | `applyItemToMessages`（materialize） | Legacy Item derive 已删；Agent Event 见 [`run-event-derive.ts`](../../../packages/agent/src/log/run-event-derive.ts)（[§3](../runtime/architecture-remediation.md) · done） |
| [`src/session/io/`](../../../packages/session/src/io/) | SessionItem ↔ jsonl；`FileSessionItemWriter` | Writer 由 Harness port 调用（[§10](../runtime/architecture-remediation.md)） |
...
| [`src/plugins/builtin/session-persistence/`](../../../packages/agent/src/plugins/builtin/session-persistence/) | Session Index · `/save` · `/resume session` · exit auto-save | — |
| [`src/context-inspect/`](../../../packages/agent/src/context-inspect/) | context 观测 · `/debug` emit | — |
| [`src/log/`](../../../packages/agent-cli/src/log/) | **Agent Event Log**（与 Session Item Log 严格区分） | — |
...
| 文档 | 关系 |
|------|------|
| [`fact-log-projections.md`](fact-log-projections.md) | Session Item Log 双源流 + materialize/compile/derive 讨论稿 |
| [`context-composer.md`](../../spec/context-composer.md) | 主 Spec（C0–C6） |
| [`architecture-remediation.md`](../runtime/architecture-remediation.md) | Session port · Phase A–C |
| [`context-window-roadmap.md`](../context/context-window-roadmap.md) | 六件事 + Budget Tiers **done** · §8 后续四条轨 · Provider D–I backlog |
...
| [`context-inspect-debug.md`](../context/context-inspect-debug.md) | `/debug` 分级全量 dump |
| [`agent-run-hooks.md`](../runtime/agent-run-hooks.md) | #2–#3 Session Observe（**done**） |
| [`utils-infrastructure.md`](../runtime/utils-infrastructure.md) | utils / storage / event-hub 分层 |
