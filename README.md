...
**当前阶段：`agent + cli` 初步可用版** — `agent` 组合根与 CLI R1/R2、R3 Settings Preflight 已完成，等待 Review；scheduler 暂缓。`llm`、`session`、`tools`、`event`、`model_input`、`context` 与 `agent-tools` 当前分期已完成；初版 draft crate 已删除，不 import、不复用。
