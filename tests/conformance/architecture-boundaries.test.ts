...
describe("architecture boundaries (structural invariants)", () => {
	...
	it("@moontide/session does not import context-composer", () => {
		const offenders = scanTsFiles(
			repoPath("packages/session/src"),
			/from\s+["']@moontide\/context-composer/,
		);
		expect(offenders).toEqual([]);
	});
	...
	it("@moontide/context-composer does not import agent/ or config", () => {
		...
		const offenders = [
			...
			...scanTsFiles(
				repoPath("packages/context-composer/src"),
				/from\s+["'].*log\//,
			),
			...scanTsFiles(
				repoPath("packages/context-composer/src"),
				/from\s+["'].*context-inspect\//,
			),
		];
		expect(offenders).toEqual([]);
	});
	...
	it("@moontide/agent log core does not import agent/ or config", () => {
		const configImport = /from\s+["'][^"']*\/config\.js["']/;
		// Only core log files (not bridge/derive adapters which are allowed to import agent/).
		const logCoreFiles = [
			"packages/agent/src/log/event-hub.ts",
			"packages/agent/src/log/run.ts",
			"packages/agent/src/log/types.ts",
			"packages/agent/src/log/enrich.ts",
			"packages/agent/src/log/persist.ts",
			"packages/agent/src/log/setup.ts",
		];
		const offenders = [
			...scanTsFiles(repoPath("packages/agent/src/log/outputs"), AGENT_IMPORT),
			...scanTsFiles(repoPath("packages/agent/src/log/outputs"), configImport),
			...scanTsFiles(
				repoPath("packages/agent/src/log/outputs"),
				/from\s+["'].*plugins\//,
			),
			...logCoreFiles.flatMap((rel) =>
				readFileSync(repoPath(rel), "utf8")
					...
					.filter(
						({ text }) =>
							AGENT_IMPORT.test(text) ||
							configImport.test(text) ||
							/from\s+["'].*plugins\//.test(text),
					),
			),
		];
		expect(offenders).toEqual([]);
	...
	it("src/ does not import monolith log core (use @moontide/agent/observability)", () => {
		const legacyCore =
	...
	it("session/ does not emit Agent Events (derive via RunEvent in packages/agent-cli)", () => {
		const offenders = scanTsFiles(
			repoPath("packages/session/src"),
			/from\s+["'].*log\/(event-hub|index|run)/,
		);
		expect(offenders).toEqual([]);
	});
	...
	it("agent/ does not import @anthropic-ai/sdk", () => {
		const offenders = scanTsFiles(
			repoPath("packages/agent/src/agent"),
			ANTHROPIC_SDK_IMPORT,
		);
		expect(offenders).toEqual([]);
	});
	...
	it("context-inspect/ does not import agent/pipeline/", () => {
		...
		const offenders = scanTsFiles(
			repoPath("packages/agent/src/context-inspect"),
			pipelineImport,
		);
		expect(offenders).toEqual([]);
	});
	...
	it("@moontide/agent does not export REPL command types from session-persistence", () => {
		const source = readFileSync(
			repoPath("packages/agent/src/index.ts"),
			"utf8",
		);
		expect(source).not.toMatch(
			/\bParsedReplParts\b|\bReplCommandResult\b|\bSessionPersistenceDeps\b/,
		);
		expect(source).not.toMatch(
			/\bhandleSaveCommand\b|\bhandleResumeSessionCommand\b/,
		);
	});

	it("@moontide/agent does not write process.stderr directly", () => {
		const offenders = scanTsFiles(
			repoPath("packages/agent/src"),
	...
	it("sidecar-host uses sidecarObservers port (not sidecarHooks)", () => {
		const source = readFileSync(
			repoPath("packages/sidecar-host/src/ports/runtime.ts"),
			"utf8",
		);
	...
	it("plugins/builtin does not import sidecar-host or plugins-sdk", () => {
		...
		const offenders = [
			...scanTsFiles(
				repoPath("packages/agent/src/plugins/builtin"),
				hostImport,
			),
	...
	it("@moontide/plugins-sdk does not import agent/", () => {
		const offenders = scanTsFiles(
			repoPath("packages/plugins-sdk/src"),
			AGENT_IMPORT,
		);
		expect(offenders).toEqual([]);
	});
	...
	it("@moontide/sidecar-host does not import agent/", () => {
		const offenders = scanTsFiles(
			repoPath("packages/sidecar-host/src"),
			AGENT_IMPORT,
		);
		expect(offenders).toEqual([]);
	});
	...
	it("tools/ does not import agent/", () => {
		const offenders = scanTsFiles(
			repoPath("packages/agent/src/tools"),
			AGENT_IMPORT,
		);
		expect(offenders).toEqual([]);
	});
	...
	it("src/ does not import monolith tools builtins (use @moontide/tools)", () => {
		const legacyImport =
			/from\s+["'](?:\.\.?\/)+tools\/(builtins|define-tool|registry)/;
		const offenders = scanTsFiles(
			repoPath("packages/agent-cli/src"),
			legacyImport,
		);
	...
	it("context/ does not import @anthropic-ai/sdk", () => {
		const offenders = scanTsFiles(
			repoPath("packages/agent-cli/src/context"),
			ANTHROPIC_SDK_IMPORT,
		);
		expect(offenders).toEqual([]);
	});

	it("errors/ does not import agent/ (except @moontide/agent/observability public subpath)", () => {
		const offenders = scanTsFiles(
			repoPath("packages/agent-cli/src/errors"),
			AGENT_IMPORT,
		).filter(({ text }) => !text.includes("@moontide/agent/observability"));
		expect(offenders).toEqual([]);
	});
	...
	it("src/ does not use instanceof Error message ternary", () => {
		...
		const allowed =
			/packages\/agent-cli\/src\/errors\/|packages\/agent\/src\/agent\/run-observers\/dispatcher\.ts|templates\/bodies\//;
		const offenders = scanTsFiles(
			repoPath("packages/agent-cli/src"),
			pattern,
	...
	it("failures.ts publishes errors via AgentEventOutputs", () => {
		const source = readFileSync(
			repoPath("packages/agent/src/agent/run-observers/failures.ts"),
			"utf8",
		);
	...
	it("tool impl files do not declare ToolSpec or defineTool(s) (§2.1)", () => {
		const specPattern =
			/\bToolSpec\b|defineTool\(|defineTools\(|defineOptionalTool\(/;
		const implFiles = [
			...
			).filter((file) => !file.endsWith(`${path.sep}tools.ts`)),
			...collectTsFiles(
	...
	it("register-defaults uses manifest factories only (no singleTool adapters)", () => {
		const source = readFileSync(
			repoPath("packages/agent/src/tools/register-defaults.ts"),
			"utf8",
		);
		expect(source).not.toMatch(/\bsingleTool\b|\boptionalSingleTool\b/);
		expect(source).not.toMatch(
			/\bdefineInspectContextTool\b|\bdefineCodeReplTool\b|\bdefineDeepResearchTool\b/,
		);
	});

	it("src/ does not hardcode legacy ocula identifiers", () => {
		const allowed =
			/packages\/shared\/src\/constants\/brand\.ts|templates\/bodies\//;
		const patterns = [/\bocula\b/i, /MOONTIDE_/, /\.ocula\b/];
		const offenders = patterns.flatMap((pattern) =>
			scanTsFiles(repoPath("packages/agent-cli/src"), pattern).filter(
	...
	it("src/ does not import monolith utils/constants/storage (use @moontide/shared)", () => {
		const legacyImport = /from\s+["'](?:\.\.?\/)+(?:utils|constants|storage)\//;
		const offenders = scanTsFiles(
			repoPath("packages/agent-cli/src"),
			legacyImport,
		);
	...
	it("src/ and tests/ do not reference legacy emitDraft identifier", () => {
		...
		const excluded = repoPath(
			"tests/conformance/architecture-boundaries.test.ts",
		);
		const offenders = [
		...
		const offenders = collectTsFiles(repoPath("packages/agent-cli/src"))
			...
			.flatMap((file) =>
				readFileSync(file, "utf8")
					...
					.filter(
						({ text }) =>
							importPattern.test(text) && !text.startsWith("import type"),
					),
			);
		expect(offenders).toEqual([]);
	});
	...
	it("ToolArgumentStatus is defined only in @moontide/shared/protocol", () => {
		...
		const offenders = packageRoots.flatMap((rel) =>
			scanTsFiles(repoPath(rel), defPattern),
		);
		expect(offenders).toEqual([]);
	});

	it("@moontide/agent root export does not leak observability API (D18)", () => {
		const source = readFileSync(
			repoPath("packages/agent/src/index.ts"),
			"utf8",
		);
		expect(source).not.toMatch(/\bAgentEvent\b/);
		expect(source).not.toMatch(/\bJsonlWriter\b/);
		expect(source).not.toMatch(/\bEventOutput\b/);
	});
});
