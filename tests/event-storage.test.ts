...
function event(runId: string, overrides: Partial<AgentEvent> = {}): AgentEvent {
	return {
...
describe("persisted event projection", () => {
	it("removes historical conversation context and duplicate tool input", () => {
		...
		const contextReport = persisted[0]?.payload.report as Record<
			string,
			unknown
		>;
...
describe("per-run gzip segments", () => {
	...
	it("rotates only between complete JSONL lines and compresses the final tail", () => {
		...
		expect(gunzipSync(fs.readFileSync(firstArchive)).toString("utf8")).toBe(
			firstLine,
		);
		...
		expect(gunzipSync(fs.readFileSync(secondArchive)).toString("utf8")).toBe(
			secondLine,
		);
	...
	it("recovers stale temp, sealed, and active files on startup", () => {
		...
		fs.writeFileSync(
			joinPath(runsDir(), "sealed-0001.jsonl.sealed"),
			sealedLine,
		);
		fs.writeFileSync(
			joinPath(runsDir(), "sealed-0001.jsonl.gz.tmp"),
			"partial",
		);
		...
		expect(fs.existsSync(joinPath(runsDir(), "sealed-0001.jsonl.sealed"))).toBe(
			false,
		);
		expect(fs.existsSync(joinPath(runsDir(), "sealed-0001.jsonl.gz.tmp"))).toBe(
			false,
		);
		expect(
			gunzipSync(
		...
		expect(fs.existsSync(joinPath(runsDir(), "active.active.jsonl"))).toBe(
			false,
		);
		expect(
			gunzipSync(
	...
	it("keeps sealed input when compression fails and recovers it later", () => {
		...
		writer.handle(first);
		writer.handle(second);
		expect(fs.existsSync(joinPath(runsDir(), "retry-0001.jsonl.sealed"))).toBe(
			true,
		);
		...
		expect(fs.existsSync(joinPath(runsDir(), "retry-0001.jsonl.gz"))).toBe(
			true,
		);
	});
	...
	it("deletes complete runs as a group while preserving active runs", () => {
		...
		expect(fs.existsSync(joinPath(runsDir(), "middle-0001.jsonl.gz"))).toBe(
			false,
		);
		expect(fs.existsSync(joinPath(runsDir(), "new-0001.jsonl.gz"))).toBe(true);
		expect(fs.existsSync(joinPath(runsDir(), "finished-0001.jsonl.gz"))).toBe(
			true,
		);
		expect(fs.existsSync(joinPath(runsDir(), "active.active.jsonl"))).toBe(
			true,
